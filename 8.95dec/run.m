...
nTries = 20;
rho = 0.870586869029240;
rho = linspace(-0.99, 0.99, nTries);
eta = linspace(0.05, 0.95, nTries);
kappa = linspace(3, 4, nTries);
fMat = zeros(nTries);
%%
% %%% Rho calibration
% for i = 1:nTries
%     for k = 1:nTries
%         [p, f, e_p] = LevenbergFuncGeneral(@funcRhoFixed,  [0.2, 2, eta(k), 0.1]', C_star, S0, K , TTM, rho(i));
%         fMat(i, k) = f;
%         i + k
%     end
% end
...
% for i = 1:nTries
%     for k = 1:nTries
%         [p, f, e_p] = LevenbergFuncGeneral(@funcRhoFixed,  [0.2, kappa(k), 0.4, 0.1]', C_star, S0, K , TTM, rho(i));
%         fMat2(i, k) = f;
%         i + k
%     end
% end
...
for k = 1:500
    ...
    rho
    objGrad
    if abs(objGrad(5)) < 10^-12
        break;
    end
end
%%
% fCali4 = zeros(nTries,1);
% pCali4 = zeros(5, nTries);
% for k = 1:nTries
...
for i = 1:1
    ...
    [C, ~] = funcRhoFixed([0.0385 2.4289 0.1181 0.0345]', S0, K , TTM, -8.8428);
