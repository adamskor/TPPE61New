function [p, f, e_p, fVec, gradVec] = LevenbergFuncGeneralBS(func, p0, target, S0, K , TTM)
...
[f_p, J_p] = func(S0, K ,0.0162, p, TTM, "call"); %f_p: n x 1, J_p: n x m
...
count = 2;
g = J_p'*e_p; %m x 1
stop = 0; eps1 = 10e-12; eps2 = 10e-45; eps3 = 10e-12;
if norm(g, 'inf') <= eps1
    stop = 1;
...
while ~stop && k < 100000
    ...
    while true
        ...
        if norm(delta_p, 2) <= eps2*norm(p, 2)
            ...
            p_new = p + delta_p; 
            [f_p_new, J_p_new] = func(S0, K, 0.0162, p_new , TTM, "call");
            change = (norm(e_p, 2)^2 - norm(target - f_p_new, 2)^2);
            rho = change/(delta_p'*(mu*delta_p + g));   
          %  if rho > 0 
                p = p_new;
%                 objGrad = objGradient(target, [p; rhoIn], S0, K, TTM);
%                 gradVec(1,count) = objGrad(1);
%                 gradVec(2,count) = objGrad(2);
%                 gradVec(3,count) = objGrad(3);
%                 gradVec(4,count) = objGrad(4);
%                 gradVec(5,count) = objGrad(5);
                A = J_p_new'*J_p_new;
                e_p = target - f_p_new;
                fVec(count) = 0.5*(e_p')*e_p;
                count = count + 1;
                g = J_p_new'*e_p ;
